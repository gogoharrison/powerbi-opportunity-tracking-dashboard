...
Based on the dashboard's interactive elements visible in the PDF — the slicers, drill-downs, navigation button, and filter buttons on both pages — here's the section to add:

---

## How to Interact with the Dashboard

The dashboard is designed for self-serve exploration. No technical background is required — all filtering and navigation is handled through point-and-click controls built directly into the report.

**Navigating Between Pages**
Use the back arrow (←) button visible on the Channel Deep Dive page to return to the Executive Overview. Page navigation tabs at the bottom of Power BI Desktop also allow direct access to either page.

**Applying Filters (Executive Overview)**
Four slicers are available at the top-right of the Executive Overview page:
- **Channel** — filter all visuals by Partner-Driven or Direct
- **Month** — isolate performance for a specific month or range of months
- **Segment** — focus on Strategic, Large, or Small & Medium accounts
- **Region** — narrow the view to East, Central, or West

Selecting any value in these slicers dynamically updates every visual on the page simultaneously.

**Filtering on the Channel Deep Dive Page**
Use the **Partner Driven** toggle (Yes / No) and the **Sales Stage** buttons (Finalize, Lead, Proposal, Qualify, Solution) to filter the scatter plot and regional breakdown table in real time. This allows targeted analysis — for example, viewing only partner-driven deals in the Qualify stage.

**Drilling Down in the Regional Table**
The regional breakdown table supports hierarchical expansion. Click the expand icon next to any region to reveal channel-level data, then expand further to view segment-level performance within that channel.

**Cross-Filtering via Charts**
Clicking on any bar, bubble, or data point within a visual will cross-filter all other visuals on the same page. For example, clicking on Sontaxon in the Top 5 Partners chart will filter the funnel, segment bars, and trend line to reflect only Sontaxon-related opportunities.

**Viewing & Downloading the Dashboard**
The Power BI report file (`.pbix`) is available in this repository. To interact with the live dashboard, download the file and open it in [Power BI Desktop](https://powerbi.microsoft.com/desktop/) (free to download). For view-only access without installation, the report can also be published to [Power BI Service](https://app.powerbi.com) and shared via a browser link.
